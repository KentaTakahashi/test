...
public final class ResourceManager {
	...
	public class ReaperThread extends Thread {
		...
		@Override
		public void run() {
			// 終了フラグが立ち、すべてのリソースが開放されるまで実行
			while(!(shutdown && refs.isEmpty())) {
				System.out.println("debug:ReaperThread running (refs size =" + refs.size() + ")");
				Reference<?> ref;
				try {
					ref = queue.remove(1000);
					if(ref == null) continue;//timeout時はcontinue
					Resource res = null;
			...
			System.out.println("debug:ReaperThread roop end");
		}
	}
